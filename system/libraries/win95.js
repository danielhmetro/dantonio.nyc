...
var defaultImage = "/system/icons/default16.png"; // default taskbar and titlebar icon (16 x 16)
...
$( document ).ready(function(){
	...
	$( "#win_container" ).on('mouseover', function() {
		$( ".win_window" ).not(".nores").resizable({
			handles: "all",
			minHeight: 150,
			minWidth: 150,
			cancel: ".nores",
			refreshPositions: true,
			iframeFix: true,
			start: function(event, ui) { // switches to window being dragged
				var win_id = $( this ).attr("id") // #window_(num)
				var win_id_splt = win_id.split("_");
				var win_num = win_id_splt[1];
				switchTo(win_num);
...
function makeWindow(icon, title, frameurl, taskbar, res, wid, hei) { /* IT WORKS!!! What's the catch? */
	...
	if (res != false) { // window is resizable
		...
		var sec1 = '<div class="win_window win_drag nores" id="window_';
		var sec2 = '" style="width: ' + wid + 'px; height: ' + hei + 'px;"';
		var sec3 = '><div class="win_titlebar win_titb_active nores" id="win_titb_';
	}
	if (icon != false) { // window has icon
		var sec4 = '"><div class="win_titb_icon"><img src="';
		var sec5 = '"></div><div class="win_titb_text">';
	} else { // window has no icon
		var sec4 = '">';
		var sec5 = '<div class="win_titb_text" style="left: 3px;">';
	}
	...
	.on('load', function () {
		...
		$(iframeWindow.document).on('click', function () {
			var winID = parentElementId.split("window_")[1]
			switchTo(winID);
		});
	})
	.appendTo( window_id );
...
function handleIframeClick() {
    // Your function to handle the iframe click
    console.log('Clicked inside iframe!');
}
// WINDOW CLOSE FUNCTION
function windowClose(num, frame) {
    //var nextWin = num--;
...
function windowBottomLeft(num) {
        var winID = "#window_" + num;
        var winHeight = $( winID ).height();
        var viewHeight = $( "#win_container" ).height();
        var heightDiff = viewHeight - winHeight - 10;
        var winStyle = "inset: " + heightDiff + "px auto auto 10px;";
        $( winID ).each(function() {
                $(this).attr("style", $(this).attr("style") + winStyle);
        });
}
